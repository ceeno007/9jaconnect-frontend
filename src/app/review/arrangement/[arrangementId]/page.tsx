...
import { createRecurringReview } from "@/lib/api/auth-client";
...
export default function RecurringReviewPage() {
  ...
  if (!loading && !isAuthenticated) {
    return (
      <PageShell title="Leave a recurring review">
        <EmptyState
          title="Sign in required"
          description="Log in as a customer to leave a review for this arrangement."
        >
          <Link href={`/login?next=/review/arrangement/${arrangementId}`}>
            <Button>Login</Button>
          </Link>
        </EmptyState>
      </PageShell>
      ...
      <PageShell title="Review submitted">
        <div className="mx-auto max-w-md ui-card p-8 text-center">
          ...
          <p className="mt-3 text-sm text-muted">
            Thanks. Your recurring review has been recorded.
          </p>
          <Link
            href={`/arrangements/${arrangementId}`}
            className="mt-6 inline-block"
          >
            <Button>Back to arrangement</Button>
          </Link>
        </div>
      </PageShell>
    );
      ...
      <div className="mx-auto max-w-xl ui-card p-6 sm:p-8">
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <label className="flex flex-col gap-1.5 text-sm">
            <span className="font-medium">Proof (optional)</span>
            <input
              type="file"
